...
ex.add_config('cfgs/my_train.yaml')
...
def train(args: Namespace, run_wandb) -> None:
    print(args)
        ...
        for k, v in model_state_dict.items():
            if k not in checkpoint_state_dict:
                ...
                #         raise NotImplementedError
                    ...
                    resume_value = checkpoint_value[list(range(0, 5))]
                    # resume_value = v
                    # print(f'Load {k} {tuple(v.shape)} from scratch.')
                else:
    ...
    if args.eval_only:
        _, coco_evaluator = evaluate(
            ...
            output_dir, visualizers['val'], args, 0, run_wandb)
        if args.output_dir:
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        train_one_epoch(
            ...
            visualizers['train'], args, run_wandb, example_ct)
        ...
        if args.eval_train:
            ...
            evaluate(
                ...
                output_dir, visualizers['train'], args, epoch, run_wandb)
        ...
        if epoch == 1 or not epoch % args.val_interval:
            val_stats, _ = evaluate(
                model, criterion, postprocessors, data_loader_val, device,
                output_dir, visualizers['val'], args, epoch, run_wandb)
            ...
            stats_dict = {'val_stats_' + k:v for k, v in zip(stat_names, val_stats)}
            best_stats_dict = {'best_stats_' + k :v for k, v in zip(stat_names, best_val_stats)}
            run_wandb.log({"epoch": epoch, **stats_dict, **best_stats_dict})
    ...
    run_wandb.log({"epoch": epoch, "total_time": total_time})
...
os.environ["WANDB_DIR"] = "wandb/bktris_1"
...
def setup_run(hyperparameters):
    ...
    if local_rank == 0:
        run = wandb.init(
            project="Tracking-vehicles-with-trackformer", config=hyperparameters,
            name= "training_1_" + str(datetime.datetime.now()))
    else:
        run = None
    return run
...
if __name__ == '__main__':
    ...
    run_wandb = setup_run(config)
    train(args, run_wandb)
    wandb.finish()
