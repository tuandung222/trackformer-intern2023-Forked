...
class Tracker:
    ...
    def add_tracks(self, pos, scores, hs_embeds, indices, labels, masks=None, attention_maps=None, aux_results=None):
    ...
    def reid(self, new_det_boxes, new_det_scores, new_det_hs_embeds, new_det_labels,
    ...
    def step(self, blob):
        ...
        if num_prev_track:
            ...
            track_labels = labels[:-self.num_object_queries]
            if 'masks' in result:
            ...
            track_keep = torch.logical_and(
                ...
                result['labels'][:-self.num_object_queries] != 4)
        ...
        new_det_labels = labels[-self.num_object_queries:]
        if 'masks' in result:
        ...
        new_det_labels = new_det_labels[new_det_keep]
        ...
        new_det_labels = new_det_labels[reid_mask]
        if 'masks' in result:
        ...
        for track in self.tracks:
            ...
            #label
            self.results[track.id][self.frame_index]['label'] = track.label.cpu().item()
...
class Track(object):
    ...
    def __init__(self, pos, score, track_id, hs_embed, obj_ind, label,
