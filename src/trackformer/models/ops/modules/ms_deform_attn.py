...
from ..functions import ms_deform_attn_core_pytorch_mot, ms_deform_attn_core_pytorch


#2 not
# MMCV = importlib.util.find_spec('mmcv')
MMCV = False
DEVICE = 'CUDA'
...
if MMCV:
    if DEVICE != 'CPU':
        ...
        from mmcv.ops.multi_scale_deform_attn import multi_scale_deformable_attn_pytorch as MSDeformAttnFunction
else:
...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_padding_mask=None, query_attn_mask=None):
        ...
        if MMCV and DEVICE == 'CUDA':
            level_start_index = torch.cat((input_spatial_shapes.new_zeros((1,)), input_spatial_shapes.prod(1).cumsum(0)[:-1]))
            output = MSDeformAttnFunction.apply(
                value, input_spatial_shapes, level_start_index, sampling_locations, attention_weights, self.im2col_step)
        elif not MMCV and DEVICE == 'CUDA':
            output = MSDeformAttnFunction.apply(
                value, input_spatial_shapes,  sampling_locations, attention_weights, self.im2col_step)
        elif MMCV and DEVICE == 'CPU':
            output = MSDeformAttnFunction(value, input_spatial_shapes, sampling_locations, attention_weights)
