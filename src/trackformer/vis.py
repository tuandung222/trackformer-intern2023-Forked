...
def vis_results(visualizer, img, result, target, tracking):
    ...
    # query_keep = keep
    # if tracking:
    #     # query_keep = keep[target['track_queries_mask'] == 0]  ---original

    #     # sjin
    #     device = keep.device  # Get the device that the `keep` tensor is on
    #     target['track_queries_mask'] = target['track_queries_mask'].to(device)  # Move the other tensor to the same device
    #     query_keep = keep[target['track_queries_mask'] == 0]  # Now you can perform the operation without error
    ...
    query_keep = keep
    if tracking:
        # Get the device that the `keep` tensor is on
        device = keep.device  
        
        # Move 'track_queries_mask' tensor to the same device
        target['track_queries_mask'] = target['track_queries_mask'].to(device)  
        
        # Now you can perform the operation without error
        query_keep = keep[target['track_queries_mask'] == 0]  
        
        # Move 'track_queries_fal_pos_mask' tensor to the same device
        target['track_queries_fal_pos_mask'] = target['track_queries_fal_pos_mask'].to(device)  
