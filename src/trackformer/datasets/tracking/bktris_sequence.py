...
DATA_ROOT = 'BKTRIS_TRAINING'

class BKTRISSequence(Dataset):
    ...
    def __init__(self, root_dir: str = 'data', seq_name: Optional[str] = None,
        ...
        self._test_folders = os.listdir(os.path.join(self._data_dir, 'val'))
    ...
    def get_seq_path(self) -> str:
        ...
        if 'train' in full_seq_name:
            return osp.join(self._data_dir, 'train', full_seq_name)
        else:
            print(osp.join(self._data_dir, 'val', full_seq_name))
    ...
    def _sequence(self) -> List[dict]:
        ...
        list_imgs = sorted(os.listdir(img_dir))
        total = [
            {'gt': boxes[i],
             'im_path': os.path.join(img_dir, list_imgs[i - 1]),
             'vis': visibility[i],
             'dets': dets[i]}
