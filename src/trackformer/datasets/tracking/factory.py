...
from .mot_wrapper import MOT17Wrapper, MOT20Wrapper, MOTS20Wrapper, BKTRISWrapper
...
names = ['seq_0_val', 'seq_1_val']
for name in names:
    DATASETS['bktris/' + name] = DATASETS[name] = (
            lambda kwargs, split=name, dets='': BKTRISWrapper(split, dets, **kwargs))
