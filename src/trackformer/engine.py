    ...
    for i, result in enumerate(results):
        ...
        if tracking and 'prev_target' in target:
            ...
            if 'track_query_match_ids' in target and len(target['track_query_match_ids']):
                target['track_query_match_ids'] = target['track_query_match_ids'].to(target['boxes'].device)    #added by sjin
                # target['boxes'] = target['boxes'].to(target['track_query_match_ids'].device)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module, postprocessors,
                    ...
                    device: torch.device, epoch: int, visualizers: dict, args, run_wandb=None, example_ct=0):
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, epoch)):
        ...
        # if is_master:
        #     run.log({"epoch": epoch, "step_loss": loss_value, **loss_dict_reduced_unscaled, **loss_dict_reduced_scaled}, step=(epoch-1) * len(data_loader) + i)
    ...
    # if is_master:
    #     run.log({"epoch": epoch, "epoch_loss": total_loss/len(data_loader)}, step=epoch * len(data_loader) + i)
    ...
    run_wandb.log({"epoch": epoch,  "epoch_loss": total_loss/len(data_loader), **{'avg_train_' + k: meter.global_avg for k, meter in metric_logger.meters.items()}, 'lr': optimizer.param_groups[0]["lr"], 'lr_backbone' :optimizer.param_groups[1]["lr"]}, step=epoch)
...
def evaluate(model, criterion, postprocessors, data_loader, device,
             output_dir: str, visualizers: dict, args, epoch: int = None, run_wandb = None):
    model.eval()
    criterion.eval()
    ...
    if run_wandb:
        run_wandb.log({"epoch_val": epoch, **{'avg_val_' + k: meter.global_avg for k, meter in metric_logger.meters.items()}}, step=epoch)  
