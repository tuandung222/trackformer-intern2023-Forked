...
global_categories = GLOBAL_CATEGORIES = [{'id': 0, 'name': 'pedestrian', 'supercategory': 'person-only'},
 {'id': 1, 'name': 'motorbike', 'supercategory': 'person-vehicle'},
 {'id': 2, 'name': 'car', 'supercategory': 'vehicle-only'},
 {'id': 3, 'name': 'truck', 'supercategory': 'vehicle-only'}]
CLASS_DICT = {str(k['id']): k['name'] for k in GLOBAL_CATEGORIES}
...
#                     label = track_data[frame_id]['label']
#                     label = CLASS_DICT[str(label)]
#                     ax.annotate(
#                         f"{track_id} - {track_data[frame_id]['obj_ind']} ({track_data[frame_id]['score']:.2f})-(class:{label})",
...
#                         color=annotate_color, weight='bold', fontsize=12, ha='center', va='center')
                if write_images == 'debug':
                    ...
                    print(1)
                    ax.annotate(
                        f"{track_id}/({track_data[frame_id]['score']:.2f})/({label})",
                        (bbox[0] + (bbox[2] - bbox[0]) / 2.0, bbox[1] + (bbox[3] - bbox[1]) / 2.0),
                        color=annotate_color, weight='bold', fontsize=12, ha='center', va='center')
