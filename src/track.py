...
ex.add_config('cfgs/my_track.yaml')
...
def main(seed, dataset_name, obj_detect_checkpoint_file, tracker_cfg,
    ...
    for seq in dataset:
        ...
        if output_dir is not None and write_images:
            _log.info("PLOT SEQ")
            plot_sequence(
                results, seq_loader, osp.join(output_dir, str(dataset_name), str(seq), f"time_{str(datetime.datetime.now())}"),
