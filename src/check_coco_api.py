...
def generate_coco_from_mot(split_name='train', seqs_names=['seq_0'],
                           root_split='train', mots=False, mots_vis=False,
                           frame_range=None, data_root='data/BKTRIS/', categories=GLOBAL_CATEGORIES):
    """
    ...
    for seq in seqs:
        ...
        if True:
            img_width = BKTRIS_INFO[seq]['img_width']
            img_height = BKTRIS_INFO[seq]['img_height']
    ...
    for seq in seqs:
        ...
        if True:
            ...
            json_file_path = os.path.join(data_root, f'{root_split}.json')
...
def check_coco_from_mot(coco_dir='data/MOT17/mot17_train_coco', annotation_file='data/MOT17/annotations/mot17_train_coco.json', img_id=None):
    ...
    i = skimage.io.imread(os.path.join(coco_dir, img['file_name']))
...
if __name__ == '__main__':
    ...
    generate_coco_from_mot(
        'train_bktris_0',
        seqs_names=['seq_0_train'])

    generate_coco_from_mot(
        'val_bktris_0',
        seqs_names=['seq_0_val'], root_split="val")
